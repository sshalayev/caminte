...
FB.prototype.runQuery = function (sql, params, callback) {
    ...
    fb.attach(this.s, function (err, db) {
        if (err) {
            throw new Error(err);
        } else {
...
FB.prototype.runExecute = function (sql, params, callback) {
    if ('function' === typeof params) {
        callback = params;
        params = [];
    }
...
FB.prototype.runTransaction = function (sql, params, callback) {
    if ('function' === typeof params) {
        callback = params;
        params = [];
    }
...
FB.prototype.create = function (name, data, callback) {
    ...
    this.runExecute(sql, params, function (err, result) {
...
FB.prototype.destroy = function destroy(name, id, callback) {
    if (id) {
        ...
        this.runExecute(sql, id, callback);
    } else {
...
FB.prototype.save = function (name, data, callback) {
    ...
    this.runExecute(sql, params, callback);
};
...
FB.prototype.all = function (name, filter, callback) {
    ...
    var eParams = params[Symbol.iterator]();
    var precookedSQL = ('SELECT ' + sql).replace(/\?/g, function(match) {
        return eParams.next().value;
    });

    console.log(precookedSQL);

    //fb.attach(this.s, function (err, db) {
    //    if (err) {
    //        throw new Error(err);
    //    } else {
    //        db.query('SELECT ' + sql, params, function (err, response) {
    //            db.detach();
    //            callback && callback(err, response);
    //        });
    //    }
    ...
    this.runQuery('SELECT ' + sql, params, callback);
                ...
                if (conds[key] === null) {
                    ...
                    switch (Object.keys(conds[key])[0]) {
                        ...
                        case 'between':
                            ...
                            break;
                        case 'in':
                            cs.push(keyEscaped + ' IN (' + val.in.join(',') + ')');
                            break;
                        case 'inq':
...
FB.prototype.count = function count(name, callback, where) {
    ...
    this.runExecute('SELECT count(*) FROM ' + quote(table) + buildWhere(where), params,
        function (err, result) {
